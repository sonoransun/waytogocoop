...
def update_phase(
    ...
    try:
        ...
        b_min = float(b_min) if b_min is not None else 0
        b_max = float(b_max) if b_max is not None else 100
        d_min = float(d_min) if d_min is not None else 0.1
        d_max = float(d_max) if d_max is not None else 10
        mu = float(mu) if mu is not None else 0
        g_factor = float(g_factor) if g_factor is not None else G_FACTOR_TSS
        resolution = int(resolution) if resolution is not None else 80
